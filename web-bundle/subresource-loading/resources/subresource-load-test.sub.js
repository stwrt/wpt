...
promise_test(async () => {
  ...
  const new_element = replaceElementWithNewRule(element, {url: '../resources/wbn/dynamic2-b1.wbn'});
...
promise_test(async () => {
  ...
  assert_equals(
    await loadScriptAndWaitReport(classic_script_url),
    'classic script from dynamic1.wbn');
  const new_element = replaceElementWithNewRule(element, {url: '../resources/wbn/dynamic2.wbn'});
  // Loading the classic script should not reuse the previously loaded
  // script. So in this case, the script must be loaded from dynamic2.wbn.
  assert_equals(
    await loadScriptAndWaitReport(classic_script_url),
  ...
  const new_element2 = replaceElementWithNewRule(new_element, {scopes: [scope + 'dummy']});
  // And in this case, the script must be loaded from network.
  assert_equals(
    await loadScriptAndWaitReport(classic_script_url),
    'classic script from network');
  // Adds the scope to hit the classic_script.js.
  const new_element3 = replaceElementWithNewRule(new_element2, {scopes: [scope + 'dummy', scope + 'classic_']});
  assert_equals(
    await loadScriptAndWaitReport(classic_script_url),
    'classic script from dynamic2.wbn');
...
function replaceElementWithNewRule(element, new_rule) {
  const new_element = replaceWebBundleRule(element, new_rule);
  element.remove();
